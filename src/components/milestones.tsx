...
export function Milestones({ milestones, cancelled }: { milestones: Milestone[]; cancelled?: boolean }) {
  return (
    <ol className="grid gap-3 sm:grid-cols-7 sm:gap-0" aria-label="Progress">
      {milestones.map((m, i) => (
          ...
          {i > 0 && (
            <span
              ...
              className={cn(
                ...
                m.state === "pending" ? "bg-ink/15" : "bg-ink",
              )}
            />
          )}
          <span
            className={cn(
              ...
              m.state === "done" && "bg-ink text-white",
              m.state === "active" && "bg-white text-ink ring-2 ring-ink",
