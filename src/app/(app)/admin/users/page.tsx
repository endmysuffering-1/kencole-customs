  ...
  return (
    <main className="mx-auto max-w-7xl p-4 sm:p-6">
      ...
      <nav className="mt-6 flex flex-wrap gap-2 text-sm" aria-label="Filter by role">
        <Link href={`/admin/users${qs({ role: undefined, cursor: undefined })}`} className={`rounded-full px-3 py-1 font-medium ${!filters.role ? "bg-ocean text-white" : "bg-white text-ink-700 ring-1 ring-inset ring-line hover:bg-paper-sunk"}`}>
          Everyone
        </Link>
        {ROLES.map((r) => (
          <Link key={r} href={`/admin/users${qs({ role: r, cursor: undefined })}`} className={`rounded-full px-3 py-1 font-medium ${filters.role === r ? "bg-ocean text-white" : "bg-white text-ink-700 ring-1 ring-inset ring-line hover:bg-paper-sunk"}`}>
