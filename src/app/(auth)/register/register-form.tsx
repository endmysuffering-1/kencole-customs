  ...
  return (
    <form onSubmit={onSubmit} className="mt-8 space-y-5">
      ...
      <fieldset>
        ...
        <div className="grid grid-cols-2 gap-2">
          {(["personal", "business"] as const).map((type) => (
            <label
              key={type}
              className={`cursor-pointer rounded-field px-3.5 py-3 text-sm ring-[1.5px] ring-inset transition ${accountType === type ? "bg-ocean text-white ring-ocean" : "bg-white ring-line-strong hover:ring-line-hover"}`}
            >
      ...
      <p className="text-center text-sm text-ink-500">
        ...
        <Link href="/login" className="font-semibold text-sky underline underline-offset-2">Sign in</Link>
      </p>
    </form>
  );
}
